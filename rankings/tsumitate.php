...
<section id="ranking-area" class="">
    <div class="ranking-title">
        <?php if(is_mobile()) : ?>
        <h2><span style="font-size: 14px;">NISA・つみたてNISAにおすすめの</span><br>証券会社ベスト<?php echo $count; ?>！</h2>
        <?php else : ?>
        <h2>NISA・つみたてNISAにおすすめの<br>証券会社ベスト<?php echo $count; ?>！</h2>
        <?php endif; ?>
    </div>
    <?php 
$i = 0;
foreach ($reslutArray as $r) {
    ...
    ?>
    <?php if($i <= 3) : ?>
    <article class="rank-0<?php echo $r['rankNisa']; ?>" data-rank="<?php echo $r['rankNisa']; ?>">
        <?php else : ?>
        <article class="rank-0<?php echo $r['rankNisa']; ?>" data-rank="4">
            <?php endif; ?>
            <div class="r-title">
